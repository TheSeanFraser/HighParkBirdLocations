...
def heatmap_layer_maker(single_species_data):
    ...
    today = date.today()
    spring_start_date = datetime.fromisoformat('2021-03-21').replace(year=today.year)
    summer_start_date = datetime.fromisoformat('2021-06-07').replace(year=today.year)
    autumn_start_date = datetime.fromisoformat('2021-08-21').replace(year=today.year)
    winter_start_date = datetime.fromisoformat('2021-10-07').replace(year=today.year)

    for entry in single_species_data:
        current_gps_coord = (entry[3], entry[4])
        entry_date = datetime.fromisoformat(entry[7]).replace(year=today.year)

        if entry_date > winter_start_date:
            winter_heatmap_list.append(current_gps_coord)
        elif entry_date > autumn_start_date:
            autumn_heatmap_list.append(current_gps_coord)
        elif entry_date > summer_start_date:
            summer_heatmap_list.append(current_gps_coord)
        elif entry_date > spring_start_date:
            spring_heatmap_list.append(current_gps_coord)

    return winter_heatmap_list, spring_heatmap_list, \
           summer_heatmap_list, autumn_heatmap_list
...
def marker_layer_maker(single_species_data):
    ...
    for entry in single_species_data:
        ...
        if entry_date > winter_start_date:
            icon = AwesomeIcon(name='circle',
                               marker_color='white',
                               icon_color='blue',
                               spin=False)

            marker_list.append(Marker(location=current_gps_coord,
                                      icon=icon,
                                      draggable=False,
...
def create_map(species, single_species_data):
    ...
    winter_gradient = {0.4: '#c2f9ff', 0.6: '#b1ebff', 0.8: '#97dfff',
                       1.0: '#007dd3'}
    spring_gradient = {0.4: '#2f5014', 0.6: '#55910e', 0.8: '#70ac0f',
                       1.0: '#b8d867'}
    summer_gradient = {0.4: '#ee0026', 0.6: '#ff004c', 0.8: '#ff001e',
                       1.0: '#ff403a'}
    autumn_gradient = {0.4: '#b2600d', 0.6: '#ea9900', 0.8: '#ffce00',
                       1.0: '#fff741'}

    winter_heatmap_locations, \
    spring_heatmap_locations, \
    summer_heatmap_locations, \
    autumn_heatmap_locations = heatmap_layer_maker(single_species_data)
    ...
    m.add_layer(winter_heatmap)
    m.add_layer(spring_heatmap)
    m.add_layer(summer_heatmap)
    m.add_layer(autumn_heatmap)
    m.add_layer(marker_cluster)

    m.save(config.dir_path + 'maps\\species\\seasons ' + species + '.html',
           title=species + ' Map')
